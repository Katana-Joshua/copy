...
class VerifyDeliveryPersonScreenState
  ...
  int docId = 0;
  List<DeliveryDocumentData>? documentList;
  bool isLoading = true;
  bool isDojahVerifying = false;
  ...
  Future<void> getDeliveryDocListApiCall() async {
    await getDeliveryPersonDocumentList().then((res) {
      ...
      deliveryPersonDocuments.forEach((element) {
        ...
        selectedDoc = remainingDocuments.validate().isNotEmpty
  ...
  addDocument(int? docId, {int? updateId}) async {
    ...
    multiPartRequest.fields["delivery_man_id"] =
    ...
    if (imageFiles != null) {
      multiPartRequest.files.add(await MultipartFile.fromPath(
          "delivery_man_document", imageFiles!.first.path));
    }
    ...
    sendMultiPartRequest(
      ...
      onSuccess: (data) async {
        ...
        // After successful upload, verify with Dojah
        try {
          var request = http.MultipartRequest(
            'POST',
            Uri.parse('$mBaseUrl/verify-document'),
          );

          request.headers.addAll({
            'Accept': 'application/json',
            'Authorization': 'Bearer ${getStringAsync(USER_TOKEN)}',
          });

          request.fields['document_id'] = docId.toString();
          request.fields['verification_type'] = 'dojah';
          request.files.add(
            await http.MultipartFile.fromPath(
                'document_image', imageFiles!.first.path),
          );
          ...
          if (response.statusCode == 200) {
            toast('Document verification initiated');
          } else {
            toast(jsonResponse['message'] ?? 'Verification failed');
          }
        } catch (e) {
          toast('Error during verification: $e');
        }
  ...
  deleteDoc(int? id, int? documentId) {
    if (!remainingDocuments.any((element) => element.id == documentId)) {
      remainingDocuments
          .add(documents.firstWhere((element) => element.id == documentId));
  ...
  Future<void> uploadDocument(DeliveryDocumentData document) async {
    ...
    if (image != null) {
      ...
      try {
        ...
        MultipartRequest multiPartRequest =
            await getMultiPartRequest('delivery-man-document-save');
        multiPartRequest.fields["id"] = '';
        multiPartRequest.fields["delivery_man_id"] =
            getIntAsync(USER_ID).toString();
        multiPartRequest.fields["document_id"] = document.id.toString();
        multiPartRequest.fields["is_verified"] = '0';
        multiPartRequest.files.add(
            await MultipartFile.fromPath("delivery_man_document", image.path));
        multiPartRequest.headers.addAll(buildHeaderTokens());

        await sendMultiPartRequest(
          multiPartRequest,
          onSuccess: (data) async {
            // Then verify with Dojah
            await verifyWithDojah(document.id.toString(), image.path);
          },
          onError: (error) {
            setState(() {
              isLoading = false;
  ...
  Future<void> verifyWithDojah(String documentId, String imagePath) async {
    setState(() {
      isDojahVerifying = true;
    });
    ...
    try {
      ...
      // Add file
      request.files.add(
        await http.MultipartFile.fromPath('document_image', imagePath),
      );
        ...
        builder: (_) => Stack(
          children: [
                          ...
                          Row(
                            ...
                            children: [
                              Text(deliveryPersonDocuments[index].documentName!,
                                  ...
                                  .expand(),
                              Text(
                                getStatus(deliveryPersonDocuments[index]
                                    .isVerified
                                    .validate()),
                                style: primaryTextStyle(
                                  color: getColor(deliveryPersonDocuments[index]
                                      .isVerified
                                      .validate()),
                                ),
                              ),
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color:
                                      ColorUtils.colorPrimary.withOpacity(0.2),
                                ...
                                getMultipleFile(
                                    ...
                                    updateId: deliveryPersonDocuments[index]
                                        .id
                                        .validate());
                              }).visible(
                                  deliveryPersonDocuments[index].isVerified !=
                                      1),
                              8.width.visible(
                                  deliveryPersonDocuments[index].isVerified !=
                                      1),
                              ...
                              Icon(Icons.verified_user, color: Colors.green)
                                  .visible(deliveryPersonDocuments[index]
                                          .isVerified ==
                                      1),
                            ],
                          ),
            ...
            emptyWidget().visible(!appStore.isLoading &&
                documents.isEmpty &&
                deliveryPersonDocuments.isEmpty),
