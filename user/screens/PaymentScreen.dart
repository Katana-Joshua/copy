...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  void flutterWaveCheckout() async {
    ...
    final String paymentOptions = "card,ussd,mpesa,ghmobilemoney,ugmobilemoney";
    print("Available payment options: $paymentOptions");
    ...
    final Flutterwave flutterwave = Flutterwave(
      ...
      customization: Customization(
          title: widget.isWallet == true
              ? "Wallet Top-up"
              : "Payment for Order #${widget.orderId}",
          description: widget.isWallet == true
    ...
    try {
      ...
      FlutterwaveViewUtils.showToast(
          context, "Payment failed: ${e.toString()}");
    }
  }
  ...
  void mobileMoneyPayment() async {
    final customer = Customer(
        name: getStringAsync(NAME),
        phoneNumber: getStringAsync(USER_CONTACT_NUMBER),
        email: getStringAsync(USER_EMAIL));

    // Configure mobile money specific options for Uganda
    final Flutterwave flutterwave = Flutterwave(
      context: context,
      publicKey: flutterWavePublicKey.validate(),
      currency: "UGX", // Force UGX for Uganda mobile money
      redirectUrl: "https://www.google.com",
      txRef: DateTime.now().millisecondsSinceEpoch.toString(),
      amount: widget.totalAmount.toString(),
      customer: customer,
      paymentOptions: "ugmobilemoney", // Specifically for Uganda mobile money
      customization: Customization(
          title: widget.isWallet == true
              ? "Mobile Money Wallet Top-up"
              : "Mobile Money Payment for Order #${widget.orderId}",
          description: widget.isWallet == true
              ? "Add money to your wallet using Mobile Money"
              : "Pay using Mobile Money (MTN or Airtel)",
          logo: "https://flutterwave.com/images/logo-colored.svg"),
      isTestMode: isTestType,
    );

    try {
      // Show mobile money provider selection dialog
      showDialog(
        context: context,
              ...
              children: [
                ListTile(
                  ...
                  onTap: () {
                    ...
                    _processMobileMoneyPayment(flutterwave, 'mtn');
                  },
                ),
                ListTile(
                  ...
                  onTap: () {
                    Navigator.pop(context);
                    _processMobileMoneyPayment(flutterwave, 'airtel');
                  },
                ),
              ],
            ),
      ...
      FlutterwaveViewUtils.showToast(
          context, "Mobile Money payment failed: ${e.toString()}");
    }
  }

  Future<void> _processMobileMoneyPayment(
      Flutterwave flutterwave, String provider) async {
    try {
      // Set the payment provider
      flutterwave.paymentOptions = provider == 'mtn' ? 'mtn' : 'airtel';

      final ChargeResponse response = await flutterwave.charge();
      print("Mobile Money Payment Response: ${response.toJson()}");
      ...
      if (response.status == 'successful') {
        Map<String, dynamic> req = {
          ...
          "reference": response.txRef.toString(),
          "payment_method":
              provider == 'mtn' ? 'mtn_mobile_money' : 'airtel_money'
        };
        ...
        FlutterwaveViewUtils.showToast(
            context, "Mobile Money payment was cancelled");
      } else {
        FlutterwaveViewUtils.showToast(context, "Mobile Money payment failed");
      }
    } catch (e) {
      print("Mobile Money Payment Error: $e");
      FlutterwaveViewUtils.showToast(
  ...
  // Add this function to prompt for mobile number
  void promptForMobileNumberAndPay() {
    String phoneNumber = '';
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: Text('Enter Mobile Money Number'),
          content: TextField(
            keyboardType: TextInputType.phone,
            onChanged: (value) => phoneNumber = value,
            decoration: InputDecoration(hintText: 'e.g. 2567XXXXXXXX'),
          ),
          actions: [
            TextButton(
              child: Text('Continue'),
              onPressed: () {
                Navigator.pop(context);
                mobileMoneyPaymentWithNumber(phoneNumber);
              },
            ),
          ],
        );
  ...
  // Modified mobile money payment to accept a phone number
  void mobileMoneyPaymentWithNumber(String phoneNumber) async {
    // Prompt for provider
    String? provider;
    await showDialog(
      context: context,
    ...
    final Flutterwave flutterwave = Flutterwave(
      ...
      isTestMode: isTestType,
      meta: {
        "mobile_money_provider": provider, // "MTN" or "AIRTEL"
      },
    );

    try {
      final ChargeResponse response = await flutterwave.charge();
      print("Mobile Money Payment Response: " + response.toJson().toString());
      ...
      if (response.status == 'successful') {
        Map<String, dynamic> req = {
          ...
          "status": response.status.toString(),
          "reference": response.txRef.toString(),
          "payment_method": provider!.toLowerCase() + "_mobile_money"
        };

        if (widget.isWallet == true) {
          paymentConfirm(
              paymentType: provider!.toLowerCase() + "_mobile_money",
              transactionId: response.transactionId);
        } else {
          savePaymentApiCall(
              paymentStatus: PAYMENT_PAID,
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              runSpacing: 16,
                              children: [
                                // Removed custom Mobile Money option
                                // Existing payment methods
                                ...paymentGatewayList.map((mData) {
